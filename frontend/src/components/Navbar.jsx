  ...
  return (
    <nav className="w-full bg-slate-950 text-slate-50 shadow-lg">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex md:items-center md:space-x-4">
            ...
            {!loggedIn && isLandingPage && (
              <>
                <button className="px-4 py-2 text-slate-50 hover:text-blue-400 transition" onClick={handleLoginClick}>
                  Login
                </button>
                <button className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-slate-50 rounded-md transition" onClick={handleSignupClick}>
                  Sign Up
                </button>
                <button className="px-4 py-2 border border-blue-500 text-blue-300 hover:bg-blue-500/10 rounded-md transition" onClick={handleDemoClick}>
                  Try Demo
                </button>
              </>
            )}
            ...
            {loggedIn && isAppPage && (
              <div className="relative ml-4" ref={dropdownRef}>
                <button className="p-2 rounded-full hover:bg-slate-700 transition cursor-pointer" onClick={() => setDropdownOpen(!dropdownOpen)}>
                ...
                {dropdownOpen && (
                  <div className="absolute right-0 mt-2 bg-slate-800 rounded-md shadow-lg py-2 z-50 min-w-[10rem]">
                    ...
                    <button className="w-full text-left px-4 py-2 hover:bg-slate-700 transition text-sm cursor-pointer" onClick={() => { navigate('/settings'); setDropdownOpen(false); setMobileMenuOpen(false); }}>Settings</button>
                    <button className="w-full text-left px-4 py-2 hover:bg-slate-700 transition text-sm cursor-pointer" onClick={() => { navigate('/help'); setDropdownOpen(false); setMobileMenuOpen(false); }}>Help / FAQ</button>
                    <button className="w-full text-left px-4 py-2 hover:bg-slate-700 transition text-sm cursor-pointer" onClick={() => { navigate('/legal/privacy-policy'); setDropdownOpen(false); setMobileMenuOpen(false); }}>Privacy Policy</button>
                    <hr className="border-slate-700 my-1" />
                    <button className="w-full text-left px-4 py-2 hover:bg-blue-700 text-slate-50 font-semibold transition rounded-b-md text-sm cursor-pointer" onClick={handleLogout}>Logout</button>
                  </div>
                )}
              </div>
            )}
          ...
          <div className="md:hidden flex items-center">
            {!isAuthPage && (
              <button className="flex flex-col justify-center items-center w-8 h-8 relative cursor-pointer" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
        ...
        <div className={`md:hidden flex flex-col items-center space-y-2 overflow-hidden transition-[max-height] duration-300 ease-in-out ${mobileMenuOpen ? 'max-h-[1000px] pb-4' : 'max-h-0'}`}>
          {loggedIn && isAppPage && (
            <form onSubmit={handleSearchSubmit} className="w-full px-2 pt-2">
              <div className="relative">
                ...
                <input type="text" placeholder="Search..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-11 pr-9 py-2 rounded-lg bg-slate-800 text-slate-50 text-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/50 border border-slate-700 transition-all" />
                {searchQuery && (
                  <button type="button" onClick={() => setSearchQuery('')} className="absolute right-5 top-1/2 transform -translate-y-1/2 text-slate-400 hover:text-slate-200 transition">
                    <X className="w-4 h-4" />
                  </button>
                )}
              </div>
          ...
          {!loggedIn && isLandingPage && (
            <>
              <button className="w-full text-center py-2 px-3 hover:text-blue-400 transition cursor-pointer" onClick={handleLoginClick}>Login</button>
              <button className="w-full text-center py-2 px-3 bg-blue-600 hover:bg-blue-700 text-slate-50 font-semibold rounded-md transition cursor-pointer" onClick={handleSignupClick}>Sign Up</button>
              <button className="w-full text-center py-2 px-3 border border-blue-500 text-blue-300 font-semibold rounded-md transition cursor-pointer" onClick={handleDemoClick}>Try Demo</button>
            </>
          )}
          {loggedIn && isAppPage && menuButtons.map(renderNavButton)}
          {loggedIn && isAppPage && (
